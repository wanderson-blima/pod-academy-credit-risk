...
O pipeline processa dados cadastrais, transacionais e comportamentais de aproximadamente **3.9 milhoes de registros** distribuidos em **6 safras** (202410 a 202503), gerando **402 colunas** consolidadas (398 features + chaves + auditoria) que alimentam modelos de Machine Learning para predicao de **First Payment Default (FPD)**.
...
graph LR
    ...
    subgraph SILVER["Silver Lakehouse<br/>5f8a4808..."]
        direction TB
        subgraph RAWDATA["schema: rawdata (21 tabelas)"]
        ...
        subgraph BOOK["schema: book (3 tabelas)"]
            S_BOOK_REC["ass_recarga_cmv<br/>90 features (REC_*)"]
            S_BOOK_PAG["pagamento<br/>94 features (PAG_*)"]
            S_BOOK_FAT["faturamento<br/>114 features (FAT_*)"]
        end
    end

    subgraph GOLD["Gold Lakehouse<br/>6a7135c7..."]
        subgraph FEATURE_STORE["schema: feature_store (2 tabelas)"]
            G_CONSOLIDADO["clientes_consolidado<br/>402 colunas<br/>~3.9M registros<br/>particionado por SAFRA"]
            G_SCORES["clientes_scores<br/>8 colunas<br/>scoring batch output"]
        end
    end
...
|---------|--------|---------|-------|----------------|
| 1. Ingestao | [`src/ingestion/ingestao-arquivos.py`](../../src/ingestion/ingestao-arquivos.py) | CSV/Excel/Parquet (Files/) | Bronze.staging.* (19 tabelas) | Leitura hibrida Pandas+Spark, audit columns (`_execution_id`, `_data_inclusao`) |
| 2. Metadados | [`src/metadata/ajustes-tipagem-deduplicacao.py`](../../src/metadata/ajustes-tipagem-deduplicacao.py) | Bronze.staging.* + config.metadados_tabelas | Silver.rawdata.* (21 tabelas) | Type casting dinamico, dedup por PK + `_data_inclusao` DESC, Delta MERGE |
| 3. Dimensoes | [`src/ingestion/criacao-dimensoes.py`](../../src/ingestion/criacao-dimensoes.py) | Parametros de data + Silver.rawdata.dados_cadastrais | Silver.rawdata.dim_calendario + dim_num_cpf | Geracao de calendario (2010-2035), surrogate keys para CPFs |
| 4a. Book Recarga | [`src/feature-engineering/book_recarga_cmv.py`](../../src/feature-engineering/book_recarga_cmv.py) | Silver.rawdata (fato + 6 dims) | Silver.book.ass_recarga_cmv (93 colunas / 90 features) | SQL template por SAFRA, broadcast joins, agregacoes por NUM_CPF |
| 4b. Book Pagamento | [`src/feature-engineering/book_pagamento.py`](../../src/feature-engineering/book_pagamento.py) | Silver.rawdata (fato + 3 dims) | Silver.book.pagamento (97 colunas / 94 features) | SQL template por SAFRA, broadcast joins, agregacoes por NUM_CPF |
| 4c. Book Faturamento | [`src/feature-engineering/book_faturamento.py`](../../src/feature-engineering/book_faturamento.py) | Silver.rawdata (fato + 2 dims) | Silver.book.faturamento (117 colunas / 114 features) | SQL template por SAFRA, SEM join com dados_cadastrais (leakage fix) |
| 5. Consolidacao | [`src/feature-engineering/book_consolidado.py`](../../src/feature-engineering/book_consolidado.py) | Silver.rawdata (base) + Silver.book (3 books) | Gold.feature_store.clientes_consolidado (402 colunas) | LEFT JOINs progressivos em (NUM_CPF, SAFRA) |
| 6. Modelo | [`src/modeling/modelo_baseline.ipynb`](../../src/modeling/modelo_baseline.ipynb) | Gold.feature_store.clientes_consolidado | Modelo treinado + metricas MLflow | Feature selection, train/val/OOT split, LogReg + LGBM |
| 7. Scoring | [`src/modeling/scoring-batch.ipynb`](../../src/modeling/scoring-batch.ipynb) | Gold.feature_store + modelo MLflow | Gold.feature_store.clientes_scores | Score 0-1000, faixas de risco (Critico/Alto/Medio/Baixo) |
| 8. Validacao | [`src/modeling/validacao-deploy.ipynb`](../../src/modeling/validacao-deploy.ipynb) | Gold.feature_store + modelo MLflow | Relatorio de validacao | Gate pre-deploy: KS/AUC/Gini vs baseline |
| 9. Monitoramento | [`src/modeling/monitoramento-drift.ipynb`](../../src/modeling/monitoramento-drift.ipynb) | Gold.feature_store + modelo MLflow | Relatorio de drift | PSI score/features + performance drift mensal |
...
#### Silver — Rawdata (21 tabelas)

As 19 tabelas do staging com tipagem correta + colunas de auditoria Silver (`_data_alteracao_silver`), mais 2 dimensoes geradas (dim_calendario, dim_num_cpf). Total: 21 tabelas.

| Tabela Adicional | Colunas | Funcao |
|-----------------|---------|--------|
| dim_calendario | 23 | Dimensao temporal (2010-01-01 a 2035-12-31) com atributos de data, semana, trimestre e flags |
| dim_num_cpf | 2 | Surrogate key (CpfKey) para CPFs unicos |

#### Silver — Book (3 tabelas)

| Tabela | Colunas DDL | Features | Prefixo Gold | Granularidade |
|--------|-------------|----------|-------------|---------------|
| ass_recarga_cmv | 93 | 90 | REC_ | NUM_CPF + SAFRA |
| pagamento | 97 | 94 | PAG_ | NUM_CPF + SAFRA |
| faturamento | 117 | 114 | FAT_ | NUM_CPF + SAFRA |

#### Gold — Feature Store (2 tabelas)

| Tabela | Colunas | Descricao | Particao |
|--------|---------|-----------|----------|
| `clientes_consolidado` | 402 | Feature store completa — 103 base + 90 REC_\* + 94 PAG_\* + 114 FAT_\* + DT_PROCESSAMENTO | SAFRA |
| `clientes_scores` | 8 | Scores de credito gerados pelo scoring batch (0-1000) | SAFRA |

**Tabela `clientes_consolidado`** (402 colunas):

| Grupo | Prefixo | Colunas | Origem |
|-------|---------|---------|--------|
| Chaves | — | 2 | NUM_CPF + SAFRA |
| Cadastrais | — | 33 | dados_cadastrais (FLAG_INSTALACAO, FPD, PROD, var_02-var_25, CEP_3_digitos, etc.) |
| Telco | — | 66 | telco (var_26-var_93) |
| Targets | TARGET_ | 2 | score_bureau_movel (TARGET_SCORE_01, TARGET_SCORE_02) |
| Recarga | REC_ | 90 | book_recarga_cmv (volumetria, valores, taxas, scores, flags) |
| Pagamento | PAG_ | 94 | book_pagamento (status, formas, alocacao, taxas, scores, flags) |
| Faturamento | FAT_ | 114 | book_faturamento (faturas, plataformas, atrasos, indices, scores, flags) |
| Auditoria | — | 1 | DT_PROCESSAMENTO |
| **Total** | | **402** | |

**Tabela `clientes_scores`** (output do scoring batch):

```sql
CREATE TABLE feature_store.clientes_scores (
    NUM_CPF       varchar(8000),
    SAFRA         int,
    SCORE_PROB    float,         -- Probabilidade de default (0-1)
    SCORE         int,           -- Score de credito (0-1000)
    FAIXA_RISCO   int,           -- Faixa: 1=Critico, 2=Alto, 3=Medio, 4=Baixo
    MODEL_NAME    varchar(8000), -- Nome do modelo usado
    MODEL_VERSION varchar(8000), -- Versao do modelo
    DT_SCORING    varchar(8000)  -- Data do scoring
);
```
...
graph TD
    ...
    subgraph BOOKS["Books de Variaveis (Silver.book)"]
        BOOK_REC["book_recarga_cmv<br/>90 features (REC_*)<br/>particionado por SAFRA"]
        BOOK_PAG["book_pagamento<br/>94 features (PAG_*)<br/>particionado por SAFRA"]
        BOOK_FAT["book_faturamento<br/>114 features (FAT_*)<br/>particionado por SAFRA"]
    end
    ...
    subgraph GOLD_FS["Gold.feature_store"]
        FS["clientes_consolidado<br/>402 colunas<br/>~3.9M registros<br/>6 SAFRAs (202410-202503)"]
    end
...
| Prefixo | Fonte | Colunas |
|---------|-------|---------|
| (nenhum) | CADASTRO + TELCO + TARGET | 103 |
| `REC_` | book_recarga_cmv | 90 |
| `PAG_` | book_pagamento | 94 |
| `FAT_` | book_faturamento | 114 |
| — | DT_PROCESSAMENTO (auditoria) | 1 |
| **Total** | | **402** |
...
graph LR
    subgraph INPUT["Feature Store"]
        FS["Gold.feature_store<br/>clientes_consolidado<br/>402 colunas<br/>~3.9M registros"]
    end
    ...
    subgraph FABRIC["Microsoft Fabric Workspace<br/>febb8631-d5c0-43d8-bf08-5e89c8f2d17e"]
        ...
        subgraph ONELAKE["OneLake Storage Layer"]
            ...
            subgraph LH_SILVER["Lakehouse Silver<br/>5f8a4808..."]
                SILVER_TABLES_RAW["Tables/rawdata/<br/>21 tabelas tipadas<br/>(19 fato/dim + 2 geradas)"]
                SILVER_TABLES_BOOK["Tables/book/<br/>3 books de features"]
            end

            subgraph LH_GOLD["Lakehouse Gold<br/>6a7135c7..."]
                GOLD_TABLES["Tables/feature_store/<br/>clientes_consolidado (402 cols)<br/>clientes_scores (8 cols)"]
            end
        end
        ...
        subgraph NOTEBOOKS["Scripts & Notebooks"]
            direction LR
            NB_INGESTAO["src/ingestion/<br/>ingestao-arquivos.py<br/>Stage 1"]
            NB_TIPAGEM["src/metadata/<br/>ajustes-tipagem.py<br/>Stage 2"]
            NB_DIMS["src/ingestion/<br/>criacao-dimensoes.py<br/>Stage 3"]
            NB_BOOKS["src/feature-engineering/<br/>book_recarga.py<br/>book_pagamento.py<br/>book_faturamento.py<br/>Stage 4"]
            NB_CONSOLIDADO["src/feature-engineering/<br/>book_consolidado.py<br/>Stage 5"]
            NB_MODELO["src/modeling/<br/>modelo_baseline.ipynb<br/>scoring-batch.ipynb<br/>monitoramento-drift.ipynb<br/>Stage 6-9"]
        end
...
```
1. src/ingestion/ingestao-arquivos.py              (CSV/Excel/Parquet -> Bronze.staging)
2. src/metadata/ajustes-tipagem-deduplicacao.py     (Bronze.staging -> Silver.rawdata)
3. src/ingestion/criacao-dimensoes.py               (-> Silver.rawdata.dim_*)
4a. src/feature-engineering/book_recarga_cmv.py     (Silver.rawdata -> Silver.book)
4b. src/feature-engineering/book_pagamento.py       (Silver.rawdata -> Silver.book)
4c. src/feature-engineering/book_faturamento.py     (Silver.rawdata -> Silver.book)
5. src/feature-engineering/book_consolidado.py      (Silver.rawdata + Silver.book -> Gold.feature_store)
6. src/modeling/modelo_baseline.ipynb               (Gold.feature_store -> modelo treinado + MLflow)
7. src/modeling/scoring-batch.ipynb                 (Gold.feature_store + modelo -> scores 0-1000)
8. src/modeling/validacao-deploy.ipynb              (Gold.feature_store + modelo -> validacao KS/AUC/Gini)
9. src/modeling/monitoramento-drift.ipynb           (Gold.feature_store + modelo -> PSI drift mensal)
```

Os estagios 4a, 4b e 4c podem ser executados em paralelo, pois nao possuem dependencias entre si.
Os estagios 7, 8 e 9 dependem do estagio 6 (modelo treinado).
...
| Metrica | Valor |
|---------|-------|
| Scripts Python (.py) | 7 (em `src/`) |
| Notebooks Jupyter (.ipynb) | 10 (em `src/modeling/` e `notebooks/`) |
| JSON Metadados Enriquecidos | 15 (em `schemas/enriched-metadata/`) |
| Tabelas Bronze (staging + config + log) | 21 |
| Tabelas Silver (rawdata + books) | 24 |
| Tabelas Gold (feature store) | 2 |
| Total de Colunas no Consolidado | 402 |
| Features Selecionadas para Modelo | 59 |
| Registros na Feature Store | ~3.9M |
| SAFRAs Processadas | 6 (202410-202503) |
| Targets | 3 (FPD, TARGET_SCORE_01, TARGET_SCORE_02) |
| Modelos Treinados | 2 (Logistic Regression L1 + LightGBM GBDT) |
| MLflow Tracking | Habilitado (v2.12.2) |
| Configuracao Centralizada | `config/pipeline_config.py` |

### Resultados do Modelo

| Metrica | LightGBM (OOT) | LR L1 (OOT) | Benchmark |
|---------|----------------|-------------|-----------|
| **KS** | **33.97%** | 32.77% | 33.10% |
| **AUC** | **0.7303** | 0.7207 | — |
| **Gini** | **46.06 pp** | 44.15 pp | — |
| **PSI** | 0.0011 | 0.0016 | — |

> LightGBM supera benchmark do Score Bureau em +0.87 pp de KS. Detalhes em [`docs/modeling/model-results.md`](../modeling/model-results.md).

---

*Documento atualizado em 2026-02-10 — Hackathon PoD Academy (Claro + Oracle) | Microsoft Fabric*
