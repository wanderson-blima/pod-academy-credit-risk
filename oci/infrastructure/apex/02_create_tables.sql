...
-- Performance comparison: LR vs LGBM side by side
CREATE OR REPLACE VIEW v_model_comparison AS
SELECT
    p1.safra,
    p1.dataset_type,
    p1.ks_statistic AS lgbm_ks,
    p1.auc_roc      AS lgbm_auc,
    p1.gini         AS lgbm_gini,
    p2.ks_statistic AS lr_ks,
    p2.auc_roc      AS lr_auc,
    p2.gini         AS lr_gini
FROM model_performance p1
JOIN model_performance p2
  ON p1.safra = p2.safra AND p1.dataset_type = p2.dataset_type
WHERE p1.model_name = 'LightGBM' AND p2.model_name = 'LR L1';
...
SELECT
    ...
    SUM(CASE WHEN drift_status = 'OK' THEN 1 ELSE 0 END) AS features_ok,
    SUM(CASE WHEN drift_status = 'WARNING' THEN 1 ELSE 0 END) AS features_warning,
    SUM(CASE WHEN drift_status = 'RETRAIN' THEN 1 ELSE 0 END) AS features_retrain,
...
SELECT
    ...
    MAX(CASE WHEN service_name = 'Database (ADW)' THEN cost_brl END) AS adw_cost,
    MAX(CASE WHEN service_name = 'Data Flow' THEN cost_brl END) AS dataflow_cost,
    MAX(CASE WHEN service_name = 'Object Storage' THEN cost_brl END) AS storage_cost
FROM cost_tracking
GROUP BY period
ORDER BY period;
