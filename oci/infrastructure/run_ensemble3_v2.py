"""
Ensemble v2 — Top-3 Champions using EXACT same feature engineering as original pipeline.
Imports feature_engineering from run_full_pipeline.py to ensure identical features.
"""
import sys, os
sys.path.insert(0, "/home/opc/ensemble3")

import pickle, json, time, uuid
import numpy as np
import pandas as pd
...
# ═══════════════════════════════════════════════════════════════
# 2. Feature Engineering — EXACT COPY from run_full_pipeline.py
#    Key: PCA fit on TRAINING data only, then transform ALL
# ═══════════════════════════════════════════════════════════════
print("\n[2/7] Feature Engineering (same as original pipeline)...")

# Convert string columns to avoid issues
for col in df.columns:
    if hasattr(df[col].dtype, 'name') and 'string' in str(df[col].dtype).lower():
        # Leave string columns as-is, just mark them
        pass
...
# --- Missing patterns ---
rec_cols = [c for c in df.columns if c.startswith("REC_") and pd.api.types.is_numeric_dtype(df[c])]
pag_cols = [c for c in df.columns if c.startswith("PAG_") and pd.api.types.is_numeric_dtype(df[c])]
fat_cols = [c for c in df.columns if c.startswith("FAT_") and pd.api.types.is_numeric_dtype(df[c])]

if rec_cols: df["MISSING_REC_COUNT"] = df[rec_cols].isnull().sum(axis=1)
if pag_cols: df["MISSING_PAG_COUNT"] = df[pag_cols].isnull().sum(axis=1)
if fat_cols: df["MISSING_FAT_COUNT"] = df[fat_cols].isnull().sum(axis=1)
if numeric_cols: df["MISSING_TOTAL_RATIO"] = df[numeric_cols].isnull().sum(axis=1) / len(numeric_cols)
...
safe_div = lambda a, b: a.fillna(0) / b.fillna(1).replace(0, 1)
if "REC_QTD_RECARGAS_TOTAL" in df.columns and "PAG_QTD_PAGAMENTOS_TOTAL" in df.columns:
    df["RATIO_REC_PAG"] = safe_div(df["REC_QTD_RECARGAS_TOTAL"], df["PAG_QTD_PAGAMENTOS_TOTAL"])
if "FAT_QTD_FATURAS_TOTAL" in df.columns and "PAG_QTD_PAGAMENTOS_TOTAL" in df.columns:
    df["RATIO_FAT_PAG"] = safe_div(df["FAT_QTD_FATURAS_TOTAL"], df["PAG_QTD_PAGAMENTOS_TOTAL"])
if "REC_QTD_RECARGAS_TOTAL" in df.columns and "PAG_QTD_PAGAMENTOS_TOTAL" in df.columns:
...
if "FAT_VLR_TOTAL_FATURAS" in df.columns and "PAG_QTD_PAGAMENTOS_TOTAL" in df.columns:
    df["BILLING_LOAD"] = safe_div(df["FAT_VLR_TOTAL_FATURAS"], df["PAG_QTD_PAGAMENTOS_TOTAL"])
...
if "REC_QTD_RECARGAS_ONLINE" in df.columns and "REC_QTD_RECARGAS_TOTAL" in df.columns:
    df["REC_ONLINE_RATIO"] = safe_div(df["REC_QTD_RECARGAS_ONLINE"], df["REC_QTD_RECARGAS_TOTAL"])
...
if "FAT_VLR_PRIMEIRA_FATURA" in df.columns and "FAT_VLR_TOTAL_FATURAS" in df.columns:
    df["FAT_FIRST_RATIO"] = safe_div(df["FAT_VLR_PRIMEIRA_FATURA"], df["FAT_VLR_TOTAL_FATURAS"])
if "REC_QTD_CANAIS_DISTINTOS" in df.columns and "REC_QTD_RECARGAS_TOTAL" in df.columns:
    df["REC_PLATFORM_DIVERSITY"] = safe_div(df["REC_QTD_CANAIS_DISTINTOS"], df["REC_QTD_RECARGAS_TOTAL"])
...
# --- PCA: FIT ON TRAINING DATA ONLY (critical!) ---
print("  PCA: fitting on training data only...")
...
for prefix, n_comp in [("REC_", 5), ("PAG_", 5), ("FAT_", 5)]:
    cols = [c for c in df.columns if c.startswith(prefix) and
            ...
            not c.endswith(("_LOG", "_SQRT", "_IS_NULL", "_COUNT", "_RATIO"))]
    if len(cols) >= n_comp:
        X_all = df[cols].fillna(0).values
        X_train = df.loc[train_mask, cols].fillna(0).values
        try:
            scaler = StandardScaler()
            scaler.fit(X_train)  # FIT ON TRAIN ONLY
            X_all_scaled = scaler.transform(X_all)  # TRANSFORM ALL

            pca = PCA(n_components=n_comp, random_state=42)
            pca.fit(X_all_scaled[train_mask.values])  # FIT ON TRAIN ONLY
            components = pca.transform(X_all_scaled)  # TRANSFORM ALL

            for i in range(n_comp):
                df[f"PCA_{prefix[:-1]}_{i}"] = components[:, i]
            print(f"    {prefix}: {len(cols)} cols -> {n_comp} PCA components (var explained: {pca.explained_variance_ratio_.sum():.3f})")
        except Exception as e:
            print(f"    PCA {prefix} failed: {e}")

new_numeric = [c for c in df.columns if pd.api.types.is_numeric_dtype(df[c])]
print(f"  After engineering: {len(new_numeric)} numeric columns (was {len(numeric_cols)})")

# ═══════════════════════════════════════════════════════════════
# 3. Load models and identify features
# ═══════════════════════════════════════════════════════════════
print("\n[3/7] Loading champion models...")
models = {}
...
if hasattr(sample_model, 'feature_names_in_'):
    ...
    for step_name, step in sample_model.named_steps.items():
    ...
    if feature_cols is None:
        ...
        if hasattr(last_step, 'feature_name_'):
            feature_cols = list(last_step.feature_name_)
        elif hasattr(last_step, 'booster_'):
            feature_cols = list(last_step.booster_.feature_name())
...
# ═══════════════════════════════════════════════════════════════
print(f"\n[4/7] Generating predictions on {len(df):,} records...")
...
for name in CHAMPION_MODELS:
    ...
    for split_name, mask in [("train", train_mask), ("oos", oos_mask), ("oot", oot_mask)]:
        ...
        if m.sum() > 0:
            ...
            gini = (2 * auc - 1) * 100
            print(f"  {split_name}: KS={ks:.4f}, AUC={auc:.4f}, Gini={gini:.2f}pp (n={m.sum():,})")
            ...
            model_metrics[name][f"gini_{split_name}"] = round(gini, 2)

    psi = compute_psi(proba[(train_mask & has_target).values], proba[(oot_mask & has_target).values])
...
# 5. Ensemble of 3
# ═══════════════════════════════════════════════════════════════
print("\n[5/7] Building Ensemble v2 (Top-3 Champions)...")

pred_matrix = np.column_stack([predictions[n] for n in CHAMPION_MODELS])
oot_m = oot_mask & has_target
oos_m = oos_mask & has_target
y_oot = df.loc[oot_m, TARGET].astype(int).values
y_oos = df.loc[oos_m, TARGET].astype(int).values

# --- Simple Average ---
print("\n  --- Simple Average (1/3 each) ---")
ens_avg = pred_matrix.mean(axis=1)
ens_avg_ks = compute_ks(y_oot, ens_avg[oot_m.values])
ens_avg_auc = roc_auc_score(y_oot, ens_avg[oot_m.values])
ens_avg_gini = (2 * ens_avg_auc - 1) * 100
ens_avg_psi = compute_psi(ens_avg[(train_mask & has_target).values], ens_avg[oot_m.values])
print(f"  OOT: KS={ens_avg_ks:.5f}, AUC={ens_avg_auc:.5f}, Gini={ens_avg_gini:.2f}pp, PSI={ens_avg_psi:.6f}")
for split_name, mask in [("train", train_mask), ("oos", oos_mask)]:
    m = mask & has_target
    ks = compute_ks(df.loc[m, TARGET].astype(int).values, ens_avg[m.values])
    auc = roc_auc_score(df.loc[m, TARGET].astype(int).values, ens_avg[m.values])
    print(f"  {split_name}: KS={ks:.5f}, AUC={auc:.5f}")
...
result = minimize(neg_ks, [1/3]*3, method="SLSQP",
                  bounds=[(0, 1)]*3,
...
ens_blend_ks = compute_ks(y_oot, ens_blend[oot_m.values])
ens_blend_auc = roc_auc_score(y_oot, ens_blend[oot_m.values])
ens_blend_psi = compute_psi(ens_blend[(train_mask & has_target).values], ens_blend[oot_m.values])
print(f"  OOT: KS={ens_blend_ks:.5f}, AUC={ens_blend_auc:.5f}")
...
coefs = {n: round(float(c), 4) for n, c in zip(CHAMPION_MODELS, meta_learner.coef_[0])}
print(f"  Meta-learner coefficients: {coefs}")

ens_stack = meta_learner.predict_proba(pred_matrix)[:, 1]
ens_stack_ks = compute_ks(y_oot, ens_stack[oot_m.values])
ens_stack_auc = roc_auc_score(y_oot, ens_stack[oot_m.values])
print(f"  OOT: KS={ens_stack_ks:.5f}, AUC={ens_stack_auc:.5f}")
...
results = {
    "simple_avg": {"ks": round(ens_avg_ks, 5), "auc": round(ens_avg_auc, 5),
                   ...
                   "weights": {n: round(1/3, 4) for n in CHAMPION_MODELS}},
...
print(f"\n  CHAMPION: {champion_name} (KS OOT = {results[champion_name]['ks']})")
...
# 6. Scoring
# ═══════════════════════════════════════════════════════════════
print(f"\n[6/7] Scoring all {len(df):,} records...")
...
champion_auc = results[champion_name]["auc"]

# Save engineered feature store
print("  Saving engineered feature store...")
eng_cols = [c for c in df.columns if pd.api.types.is_numeric_dtype(df[c]) or c in ["NUM_CPF", "SAFRA"]]
df[eng_cols].to_parquet(os.path.join(OUTPUT_DIR, "feature_store_engineered.parquet"), index=False)
...
ensemble_output = {
    ...
    "ks_oot": champion_ks,
    "auc_oot": champion_auc,
    "gini_oot": results[champion_name].get("gini", round((2*champion_auc-1)*100, 2)),
    "psi": results[champion_name].get("psi"),
    "all_results": results,
    ...
    "scoring": {
        ...
        "distribution": {f: int(dist.get(f, 0)) for f in ["CRITICO", "ALTO", "MEDIO", "BAIXO"]},
    },
...
# Save ensemble model
class EnsembleTop3:
    def __init__(self, base_models, weights, champion_method, feature_cols):
    ...
    def predict_proba(self, X):
        preds = np.column_stack([self.base_models[n].predict_proba(X)[:, 1] for n in self.model_names])
        if self.champion_method == "stack" and self.meta_learner:
            proba = self.meta_learner.predict_proba(pd.DataFrame(preds, columns=self.model_names))[:, 1]
        else:
            w = [self.weights[n] for n in self.model_names] if self.weights else None
            proba = np.average(preds, axis=1, weights=w)
        return np.column_stack([1 - proba, proba])

ensemble_v2 = EnsembleTop3(models, results[champion_name].get("weights"), champion_name, feature_cols)
...
for fname in sorted(os.listdir(OUTPUT_DIR)):
    ...
    unit = "MB" if fsize > 1024*1024 else "KB"
    val = fsize/1024/1024 if unit == "MB" else fsize/1024
    print(f"    {fname} ({val:.1f} {unit})")

# ═══════════════════════════════════════════════════════════════
print("\n" + "=" * 60)
print("  ENSEMBLE v2 (TOP-3 CHAMPIONS) — FINAL RESULTS")
print("=" * 60)
print(f"  Champion method: {champion_name}")
print(f"  Models: {', '.join(CHAMPION_MODELS)}")
print(f"  KS OOT:  {champion_ks:.5f}")
print(f"  AUC OOT: {champion_auc:.5f}")
print(f"  Gini OOT: {ensemble_output['gini_oot']:.2f}pp")
print(f"  PSI: {ensemble_output.get('psi', 'N/A')}")
